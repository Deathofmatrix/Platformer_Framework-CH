...
namespace CharlieHarrop
{
    ...
    public class SceneLoader : MonoBehaviour
    {
        ...
        //This is taking the flag and checking if a player has collided with it and then loading the same level to restart
        private void OnTriggerEnter2D(Collider2D collision)
        {
            if (collision.tag == "Player")
            {
                LoadScene(sceneToLoad);
                SimpleCharacterController.runSpeed = 1f;
                SimpleCharacterController.jumpHeight = 1f;
            }
